 ...
 * <code>Mercadoria</code> pelo campo
 * <code>nome</code>.
 *
 * <p>
 * <code>BuscarMercadoriaController</code> é mapeada como
 * <code>@Component</code> do Spring. Dessa forma uma instância de <code>BuscarMercadoriaController</code> pode ser criada e gerenciada pelo Spring, favorecendo
 * a Inversão de Controle <i>(IoC)</i> e Injeção de Dependência <i>(DI)</i>.
 * </p>
 *
 * <p>
 * Essa <code>Controller</code> depende de outros componentes da camada <code>Model</code> (DAO) e <code>View</code> (tela). As dependências são resolvidas pelo
 * Spring, através da <strong>Injeção de Dependência</strong> c/ a anotação <code>@Autowired</code>.
 * </p>
 *
...
//                    list = dao.getMercadoriasByNome(frame.getText().concat("%"));
                    list = mercadoriaService.getByNome(frame.getText());
                }
            }
